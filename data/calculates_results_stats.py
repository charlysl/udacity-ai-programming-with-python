...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic["n_dogs_img"] = sum([val[3] for val in results_dic.values()])
    ...
    results_stats_dic["n_match"] = sum([val[2] for val in results_dic.values()])

    #number of correctly classified dog images
    results_stats_dic["n_correct_dogs"] = sum([val[4] for val in results_dic.values() if val[3]])

    #number of correctly classified NON-dog images
    results_stats_dic["n_correct_notdogs"] = len([val for val in results_dic.values() if not val[3] and not val[4]])

    #number of correctly classified dog breeds
    results_stats_dic["n_correct_breed"] = sum([val[2] for val in results_dic.values() if val[3]])
